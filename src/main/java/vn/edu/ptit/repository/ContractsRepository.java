...
public interface ContractsRepository extends JpaRepository<Contracts, Long> {
    ...
    @Modifying
    @Query("Update Contracts c Set c.status = 'EXPIRED' where c.endDate < CURRENT_DATE and c.status <> 'EXPIRED'")
    int updateExpiredContracts();

    @Query("SELECT distinct c FROM Contracts c WHERE (c.landLord.id = :userId OR c.customer.id = :userId) AND (c.endDate is null OR c.endDate >= CURRENT_DATE) ORDER BY c.createdAt DESC")
    List<Contracts> findValidContractsForUser(@Param("userId") Long userId);

    @Query("SELECT distinct c FROM Contracts c WHERE (c.landLord.id = :userId OR c.customer.id = :userId) AND c.endDate < CURRENT_DATE ORDER BY c.createdAt DESC")
